...
def payment(order_id):
    ...
    if request.method == "POST":
        ...
        if method == "GCash":
            result = initiate_gcash_payment(order["total"], order_id, f"Order {order_id}")

        elif method == "PayMaya":
            result = initiate_paymaya_payment(order["total"], order_id, f"Order {order_id}")
...
def payment_success(order_id):
    flash("Payment simulated as successful.", "success")
    return redirect(url_for("home"))
...
@admin_required
def edit_tutorial():
    t = Tutorial.query.get_or_404(request.form.get("tutorial_id"))
...
def delete_tutorial(tutorial_id):
    db.session.delete(Tutorial.query.get_or_404(tutorial_id))
    db.session.commit()
